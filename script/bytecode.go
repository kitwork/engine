...
var kitworkPlugin = api.Plugin{
	...
	Setup: func(build api.PluginBuild) {
		...
		build.OnLoad(api.OnLoadOptions{Filter: `.*`, Namespace: "kitwork-ns"}, func(args api.OnLoadArgs) (api.OnLoadResult, error) {
			...
			switch args.Path {
			case "kitwork":
				contents = `export const { router, log, render, http, database, go, qrcode, screenshot, chromedp, napas, file, browser, jwt } = kitwork();`
			case "kitwork/router":
			...
			case "kitwork/qrcode":
				contents = `export const qrcode = kitwork().qrcode; export default qrcode;`
			case "kitwork/screenshot":
				contents = `export const screenshot = kitwork().screenshot; export default screenshot;`
			case "kitwork/chromedp":
			...
			case "kitwork/browser":
				contents = `export const browser = kitwork().browser; export default browser;`
			case "kitwork/jwt":
				contents = `export const jwt = kitwork().jwt; export default jwt;`
			default:
