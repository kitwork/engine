...
func Stress(source string, iterations int) (value.Value, error) {
	return New().Stress(source, iterations)
}
...
func (s *Script) Stress(source string, iterations int) (value.Value, error) {
	code, err := s.code(source)
	if err != nil {
		return value.Value{K: value.Invalid}, err
	}
