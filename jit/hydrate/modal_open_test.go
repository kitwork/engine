...
// This is the test the DEMO earned — it composes data-kit-if + data-kit-click:outside + a component method
...
//  2. The opening click's own outside handler: the click that runs open() mounts the modal (which
//     carries data-kit-click:outside); the SAME click then reaches that listener, and the trigger is "outside" the
//     just-mounted panel — so the modal closed itself instantly. markFresh/isFresh fix that.
//
// A later, separate outside click must still close it (:outside is not broken, just click-scoped).
...
var panel = el("div", { "data-kit-click:outside": "adding = false", "data-kit-keydown:escape:window": "adding = false" });
...
function mounted() { return document.querySelectorAll("[data-kit-click:outside]").length > 0; }

if (mounted()) throw new Error("initial: modal must be closed");

// The opening click: runs open() → mounts the modal, and must NOT be closed by its own :outside check.
...
// click's own :outside handler fired). Either regression leaves the modal absent here.
...
// A genuinely separate later outside click DOES close it (:outside still works). freshMounts clears on a
...
setTimeout(function () {
  ...
  if (mounted()) throw new Error(":outside: a separate outside click should close the modal");
  console.log("modal open flow: if(component scope) mounts on click, survives its own :outside, closes on a later outside click");
}, 0);
`
