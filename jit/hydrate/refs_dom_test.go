...
// data-kit-ref + `$refs` in the kernel (ideaship-final §6): a ref belongs to the boundary that owns
// it, so `$refs.field` inside a scope is that scope's field, on the page it is the page's, a nested
...
func TestKernelRefsAreScopedToTheActingBoundary(t *testing.T) {
	runNodeDOMScript(t, "refs.js", `
var kit = window.kit;
var pageField = el("input", { "data-kit-ref": "field", "value": "page" });
var pageRead = el("button", { "data-kit-click": "hit = $refs.field.getAttribute('value')" });
...
var outerField = el("input", { "data-kit-ref": "field", "value": "outer" });
var outerRead = el("button", { "data-kit-click": "found = $refs.field.getAttribute('value'); missing = $refs.nothing == null ? 'null' : 'element'" });
var outerInner = el("button", { "data-kit-click": "missing = $refs.inner == null ? 'null' : 'element'" });
...
var innerField = el("input", { "data-kit-ref": "inner", "value": "inner" });
var innerOuter = el("button", { "data-kit-click": "seen = $refs.field == null ? 'null' : 'element'" });
var innerOwn = el("button", { "data-kit-click": "seen = $refs.inner.getAttribute('value')" });
...
click(outerRead);
if (found.textContent !== "outer") throw new Error("$refs.field inside the scope should be the scope's own field, got " + JSON.stringify(found.textContent));
if (missing.textContent !== "null") throw new Error("a missing name should be nullish, got " + missing.textContent);
click(pageRead);
if (pageOut.textContent !== "page") throw new Error("$refs.field on the page should be the page-level field, got " + JSON.stringify(pageOut.textContent));
click(outerInner);
