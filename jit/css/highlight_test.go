...
// Registered tokens must emit the themeable var() form. Baked hex would make
// terminal-* the only family on a site that a [data-theme] block cannot re-skin.
func TestTerminalTokensEmitTheThemeableVarForm(t *testing.T) {
	stubPalette(t)
	got := colorCSSValue("terminal", "", "", nil)
	if !strings.Contains(got, "var(--color-terminal,") {
		t.Fatalf("terminal emitted %q, want the var() form every configured token gets", got)
	}
	if !strings.Contains(got, Hex("#1a1b26").String()) {
		t.Fatalf("the var() fallback lost the colour: %q", got)
	}
}
