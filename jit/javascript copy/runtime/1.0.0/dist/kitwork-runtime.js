/* Kitwork Client Runtime — Unified M1-M5 1.0.0-draft
...
var morphModule = __require(23);
var headModule = __require(24);
var driveModule = __require(25);
...
var RESERVED_ATTRIBUTES = new Set([
  ...
  "data-kit-drive", "data-kit-no-drive", "data-kit-prefetch", "data-kit-reload",
  "data-kit-once", "data-kit-script", "data-kit-head", "data-kit-scroll-preserve",
  "data-kit-drive-pending"
]);
...
function createRuntime(globalObject, options) {
  ...
  runtime.morph = morphModule.createMorphEngine(runtime);
  runtime.head = headModule.createHeadManager(runtime);
  runtime.drive = driveModule.createDriveManager(runtime);
  ...
  runtime.hydrateTree = function hydrateTree(node, app) {
    ...
    if (node === app.root && runtime.drive) runtime.drive.attach(app);
    record.hydrated = true;
  ...
  function setupObserver(app) {
    ...
    if (app.observer) {
      if (!app.observerPauseDepth) app.observer.observe(app.root, app.observerOptions);
      return;
    }
    app.observerOptions = { childList: true, subtree: true, attributes: true };
  ...
  runtime.pauseObserver = function (app) {
    if (!app || !app.observer) return;
    app.observerPauseDepth = (app.observerPauseDepth || 0) + 1;
    if (app.observerPauseDepth === 1) {
      app.observer.disconnect();
      if (typeof app.observer.takeRecords === "function") app.observer.takeRecords();
    }
  };

  runtime.resumeObserver = function (app) {
    if (!app || !app.observer) return;
    app.observerPauseDepth = Math.max(0, (app.observerPauseDepth || 0) - 1);
    if (!app.observerPauseDepth && !app.destroyed) {
      if (typeof app.observer.takeRecords === "function") app.observer.takeRecords();
      app.observer.observe(app.root, app.observerOptions || { childList: true, subtree: true, attributes: true });
    }
  };
  ...
  runtime.destroyApp = function (app) {
    ...
    runtime.drive.detach(app);
    if (app.observer) app.observer.disconnect();
    runtime.cleanupTree(app.root, app);
    while (app.cleanups.length) {
      try { app.cleanups.pop()(); }
      catch (cleanupError) { runtime.reportError(cleanupError, { app: app, phase: "app-cleanup" }); }
    }
    app.aliases.clear();
    app.bindings.clear();
  ...
  runtime.services.register("drive", runtime.drive, {
    expression: ["visit", "prefetch", "back", "forward", "reload", "enabled"]
  });
  ...
  kit.internal = Object.freeze({
    ...
    morph: runtime.morph,
    drive: runtime.drive,
    pauseObserver: runtime.pauseObserver,
    resumeObserver: runtime.resumeObserver,
...
var api = __require(27);
if (global) global.KitworkRuntime = api;
