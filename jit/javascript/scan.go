...
func validateReservedAttribute(tagName string, attribute rawScannedAttribute) error {
	...
	// is server-only — the pass consumes it while rendering and the browser
	// never acts on it — so it is declared where the work happens.
	...
	// since it shipped. Only the staged DELIVERY roles are forgeable — a
	// <script> claiming to be the runtime would smuggle code into a pipeline
	// whose whole point is content-addressed integrity. reservedDeliveryMarker
	// already draws exactly that line for the injection guard, so ask it rather
	// than keeping a second, blunter copy of the rule here.
