/*! @kit/kitjs v1.0.0 | MIT | https://kit.io | generated from engine/jit/hydrate/runtime.js — do not edit */
// kit kernel — THE one client runtime. Served at /jithydrate, and prepended by jit/js as the
// core of every verb bundle, so expressions, verbs, model, validate and live all ride ONE root
// (window.kit), ONE registry, ONE set of delegated listeners, ONE observer.
//
// Boot-guarded: safe under double inclusion and under kit Drive re-running head scripts.
// Authors write data-kit-*/data-kit-* SOURCE attributes; this kernel carries a tiny parser for
// that source (same grammar the Go side compiles for ctx.validate) plus the IR walker. It also
// reads data-kit-*-ir when present (optional precompiled mode). No eval, no new Function, ever.
//
...
(function () {
    ...
    // Component auto-loading is OPT-IN: set kit.cdnComponents to a base URL (e.g. an npm CDN like
    // jsdelivr) BEFORE the kernel boots, and an unregistered data-kit-component is fetched from
    // <base>/<name>/<version|name>.js. Default OFF — a kit-engine page delivers components itself
    ...
    var bridge = window.kitBridge || null;
    kit.bridge = bridge;
    kit.platform = bridge && bridge.platform ? bridge.platform : "web";
    kit.isNative = !!bridge;

    if (kit.runtime) return;
    kit.runtime = "v.1.0.0";
    ...
    function directive(el, name) {
        var raw = el.getAttribute("data-kit-" + name + "-ir");
        if (raw) {
            if (!(raw in cache)) { try { cache[raw] = JSON.parse(raw); } catch (e) { cache[raw] = null; } }
            return cache[raw];
        }
        raw = el.getAttribute("data-kit-" + name) || el.getAttribute("data-kit-" + name);
        if (!raw) return null;
        var key = "$" + raw;
    ...
    function selector(name) {
        return "[data-kit-" + name + "],[data-kit-" + name + "],[data-kit-" + name + "-ir]";
    }

    var MODEL = "[data-kit-model],[data-kit-model]";
    function modelKey(el) { return el.getAttribute("data-kit-model") || el.getAttribute("data-kit-model"); }
    ...
    var SCOPE = "[data-kit-scope],[data-kit-scope],[data-kit-component],[data-kit-component],[data-kit-api],[data-kit-api]";
    ...
    function boundaryScope(b) {
        var st = state(b);
        var cname = b.getAttribute("data-kit-component") || b.getAttribute("data-kit-component");
        if (cname) {
        ...
        st.scope = {};
        var v = (b.getAttribute("data-kit-scope") || b.getAttribute("data-kit-scope") || "").trim();
    ...
    function rebuildActiveComponents() {
        var next = {};
        document.querySelectorAll("[data-kit-component],[data-kit-component]").forEach(function (el) {
            var cname = el.getAttribute("data-kit-component") || el.getAttribute("data-kit-component");
            if (!cname) return;
            var st = state(el);
            if (st.seeded) {
    ...
    // accepted; put it on the root, or spread across elements) — or kit.remember("theme"). Those
    ...
    var REMEMBER = "[data-kit-remember],[data-kit-remember]";
    var remembered = {};
    var memCache = {};
    ...
    Object.defineProperty(kit, "theme", {
        get() {
    ...
    function setTheme() {
        ...
        var comp = window.kit.scope.theme;
        if (comp) comp.dark = dark;
    }

    function loadRemembered() {
        document.querySelectorAll(REMEMBER).forEach(function (el) {
            parseKeys(el.getAttribute("data-kit-remember") || el.getAttribute("data-kit-remember")).forEach(registerRememberedKey);
        });
    }
    ...
    var behaviors = {};
    var stateKey = Symbol("kit");
    function state(element) {
        return element[stateKey] || (element[stateKey] = {});
    }
    // data-kit-target = "#id"/selector → element; defaults to the actor itself.
    function target(el) {
        var sel = el.getAttribute("data-kit-target") || el.getAttribute("data-kit-target");
        return sel ? document.querySelector(sel) : el;
    }
    var ACTION = "[data-kit-action],[data-kit-action]";
    function fire(el, e) {
        var fn = behaviors[el.getAttribute("data-kit-action") || el.getAttribute("data-kit-action")];
        if (fn) fn(el, e);
    }
    kit.behavior = function (name, fn) { behaviors[name] = fn; return kit; };
    ...
    kit.component = function (name, def) { blueprints[name] = def; scheduleRender(); return kit; };
    ...
    kit.action = function (name, fn) { behaviors[name] = fn; return kit; };
    ...
    // Back-compat surface for existing verb modules and pages (kit.components.action(...)).
    ...
    document.addEventListener("submit", function (e) {
        ...
        if (f.getAttribute && (f.getAttribute("data-kit-action") || f.getAttribute("data-kit-action"))) fire(f, e);
    }, true);

    // Auto-trigger: [data-kit-trigger="visible"] fires its action when scrolled into view (lazy
    // load / infinite scroll). Re-evaluated on every kit:load (after navigation or an append).
    function bindVisible() {
        if (!("IntersectionObserver" in window)) return;
        document.querySelectorAll('[data-kit-trigger="visible"],[data-kit-trigger="visible"]').forEach(function (el) {
            var store = state(el);
    ...
    document.addEventListener("kit:load", bindVisible);
    ...
    var LIVE = "[data-kit-live],[data-kit-live]";
    var streams = {};
    ...
    function syncLive() {
        ...
        document.querySelectorAll(LIVE).forEach(function (el) {
            var u = el.getAttribute("data-kit-live") || el.getAttribute("data-kit-live");
    ...
    function morph(fromNode, toNode) {
        ...
        if (fromNode.nodeType === 1) {
            ...
            function getKey(n) {
                return n.nodeType === 1 ? (n.getAttribute("data-kit-key") || n.getAttribute("data-kit-key") || n.getAttribute("data-key")) : null;
            }
    ...
    // hydrate region ([data-kit-hydrate] / [data-kit-hydrate]) AND no drive is already running:
    ...
    // Contract unchanged: intercept same-origin links + GET forms, fetch with X-kit-Hydrate,
    // swap the region (fallback <main>), morph, mergeHead swaps data-kit-jit blocks, history +
    ...
    function initDrive() {
        ...
        var appEl = document.querySelector("[data-kit-app],[data-kit-app],[data-kit-hydrate],[data-kit-hydrate]");
        if (!appEl) return;
        kit.hydrate = true;
        ...
        function region(doc) { return doc.querySelector("[data-kit-app],[data-kit-app],[data-kit-hydrate],[data-kit-hydrate]") || doc.querySelector("main"); }
        function layoutKey(doc) { return doc.body ? (doc.body.getAttribute("data-kit-layout") || "") : ""; }
        ...
        function drivable(a, e) {
            ...
            if (a.getAttribute("data-kit-app") === "false" || a.getAttribute("data-kit-app") === "false" ||
                a.getAttribute("data-kit-hydrate") === "false" || a.getAttribute("data-kit-hydrate") === "false") return false;
            if (a.closest && (
                a.closest("[data-kit-app='false'],[data-kit-app='false']") ||
                a.closest("[data-kit-hydrate='false'],[data-kit-hydrate='false']")
            )) return false;
            if (a.getAttribute("data-kit-action") || a.getAttribute("data-kit-action")) return false; // verbs own their triggers
            if (a.getAttribute("data-kit-click") || a.getAttribute("data-kit-click") || a.getAttribute("data-kit-click-ir")) return false; // expression links don't navigate
        ...
        function fetchPage(url, signal) {
            return fetch(url, { headers: { "X-kit-Hydrate": "1" }, signal: signal, credentials: "same-origin" })
                .then(function (r) {
                    var ct = r.headers.get("content-type") || "";
                    if (!r.ok || ct.indexOf("text/html") === -1) throw new Error("not drivable: " + r.status);
                    var layout = r.headers.get("X-kit-Layout") || "";
        ...
        function mergeHead(doc) {
            ...
            document.head.querySelectorAll("style[data-kit-jit]").forEach(function (n) { n.remove(); });
            doc.head.querySelectorAll("style[data-kit-jit]").forEach(function (n) {
                document.head.appendChild(n.cloneNode(true));
            });
            var oldRun = document.head.querySelector('script[data-kit-jit="js"]');
            if (oldRun) oldRun.remove();
            var newRun = doc.head.querySelector('script[data-kit-jit="js"]');
            if (newRun) {
                var run = document.createElement("script");
                run.setAttribute("data-kit-jit", "js");
        ...
        function swap(html, url, push, scrollY, responseLayout) {
            ...
            var newAppEl = doc.querySelector("[data-kit-app],[data-kit-app],[data-kit-hydrate],[data-kit-hydrate]");
            if (newAppEl) {
                var newAppVal = newAppEl.getAttribute("data-kit-app") || newAppEl.getAttribute("data-kit-app") || "";
            ...
            var cur = sameLayout ? region(document) : document.querySelector("[data-kit-shell]");
            var next = sameLayout ? region(doc) : doc.querySelector("[data-kit-shell]");
            ...
            document.dispatchEvent(new CustomEvent("kit:before-swap", { detail: { url: url } }));
            if (doc.title) document.title = doc.title;
            mergeHead(doc);
            morph(cur, next);
            if (!sameLayout && doc.body) {
                document.body.className = doc.body.className;
                document.body.setAttribute("data-kit-layout", layoutKey(doc));
            }
            runScripts(cur);

            if (push) history.pushState({ kit: true }, "", url);
            ...
            document.dispatchEvent(new CustomEvent("kit:load", { detail: { url: url } }));
        }
        ...
        function visit(url, push, scrollY) {
            ...
            document.documentElement.classList.add("kit-loading");
            progress(true);
            document.dispatchEvent(new CustomEvent("kit:before-visit", { detail: { url: url } }));
            ...
            p.then(function (res) {
                ...
                .then(function () {
                    document.documentElement.classList.remove("kit-loading");
                    progress(false);
                    inflight = null;
                });
        }
        ...
        document.addEventListener("click", function (e) {
            ...
            history.replaceState({ kit: true, y: window.scrollY }, "", location.href);
            visit(a.href, true, 0);
        });
        ...
        document.addEventListener("submit", function (e) {
            ...
            if (f.getAttribute("data-kit-app") === "false" || f.getAttribute("data-kit-app") === "false" ||
                f.getAttribute("data-kit-hydrate") === "false" || f.getAttribute("data-kit-hydrate") === "false") return;
            if (f.getAttribute("data-kit-action") || f.getAttribute("data-kit-action")) return;
        ...
        window.addEventListener("scroll", function () {
            ...
            history.pushState({ kit: true, trap: true }, "", location.href);
            trapped = true;
        }, { passive: true });
        ...
        history.replaceState({ kit: true }, "", location.href);
    }
    ...
    kit.run = function (ir, s) { return run(ir, s || scope); };
    kit.scope = scope;
    ...
    function initAppConfig() {
        var appEl = document.querySelector("[data-kit-app],[data-kit-app],[data-kit-hydrate],[data-kit-hydrate]");
        if (appEl) {
            var appVal = appEl.getAttribute("data-kit-app") || appEl.getAttribute("data-kit-app") || "";
            var mode = "runtime";
            var version = "latest";
            if (appVal) {
            ...
            kit.mode = mode;
            kit.version = version;
            kit.useIndexed = appEl.getAttribute("data-kit-indexed") === "true" || appEl.getAttribute("data-kit-indexed") === "true";
        }
    }
    ...
    // (bindVisible re-binds through its own kit:load listener above).
    document.addEventListener("kit:load", function () { loadRemembered(); seedModels(); render(); syncLive(); syncApi(); });
})();