...
<html lang="en"><head><meta charset="utf-8"><title>KitJS system variables</title></head><body>
  <section id="boundary" data-kit-scope="thisId: '', sameEl: 'unset', hostId: '', targetId: '', submitterId: '', eventType: ''">
    <button id="probe" type="button" data-kit-click="thisId = $this.id; sameEl = $this === $el ? 'yes' : 'no'; hostId = $host.id; targetId = $event.target.id; eventType = $event.type"><span id="inner">probe</span></button>
...
__runStandaloneKitTest(async function () {
  ...
  byId("inner").click();
  await waitFor(function () { return byId("out").textContent.indexOf("probe yes boundary inner click") === 0; }, "system variables in a scoped action, got " + JSON.stringify(byId("out").textContent));
  byId("send").click();
