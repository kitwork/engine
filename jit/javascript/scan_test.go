...
func TestScanHTMLDetectsImplementedRuntimeDirectivesAndEvents(t *testing.T) {
	...
	tests := []string{
		...
		`<input data-kit-element="search"><button data-kit-click="$element.search.focus()"></button>`,
...
func TestScanHTMLRejectsUnsupportedReservedAttributes(t *testing.T) {
	...
	for _, source := range []string{
		...
		// data-kit-element names an element for $element.<name>: an identifier, one per element, no modifiers.
		`<div data-kit-element></div>`,
		`<div data-kit-element=""></div>`,
		`<div data-kit-element="$field"></div>`,
		`<div data-kit-element="search-box"></div>`,
		`<div data-kit-element="constructor"></div>`,
		`<div data-kit-element="document"></div>`,
		`<div data-kit-element:once="field"></div>`,
