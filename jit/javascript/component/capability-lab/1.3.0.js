...
// KitJS managed component: capability-lab@1.3.0
...
var DEFINITIONS = Object.freeze([
  ...
  Object.freeze({ id: "notification-tap", label: "Notification tap route", api: "notifications.show", group: "Native", capability: "notifications.tap", fallback: false }),
...
var notificationTapArms = new WeakMap();
var NOTIFICATION_TAP_PATH = "/capability-lab?source=notification#tap";
...
function receiveDeepLink(scope, snapshot, label) {
  scope.deepLinkPath = snapshot.path;
  scope.record("deep-links", "pass", "granted", "received");
  var arm = notificationTapArms.get(scope);
  if (arm && snapshot.id !== arm.baselineID && snapshot.path === NOTIFICATION_TAP_PATH) {
    notificationTapArms.delete(scope);
    scope.record("notification-tap", "pass", "granted", "matching-route-received");
    scope.statusText = "Đã nhận route mới khớp notification đã arm. Kết quả chỉ xác nhận route trong inbox, không xác nhận nguồn hay thao tác chạm vật lý.";
    return true;
  }
  scope.statusText = label + snapshot.path;
  return false;
}
...
kit.component("capability-lab", {
  ...
  init: function (context) {
    ...
    if (subscribeDeepLinks) {
      stopDeepLinks = subscribeDeepLinks.call(kit.deepLinks, function (snapshot) {
        if (!scope.active) return;
        receiveDeepLink(scope, snapshot, "Đã nhận deep link nội bộ đã chuẩn hóa: ");
      });
    }
  ...
  checkDeepLink: function () {
    ...
    return Promise.resolve().then(function () {
      ...
      receiveDeepLink(scope, value, "Snapshot deep link hiện tại: ");
      return true;
    }, function (error) {
      if (scope.active) {
  ...
  showNotification: function () {
    ...
    var scope = this;
    var supports = method("capabilities.supports");
    var tapRouteAttached = false;
    var tapBaselineID = null;
    notificationTapArms.delete(this);
    this.notificationBusy = true;
    ...
    return Promise.resolve().then(function () {
      ...
      return Promise.resolve().then(function () {
        return supports.call(kit.capabilities, "notifications.tap");
      }).then(function (value) { return value === true; }, function () { return false; });
    }).then(function (canRouteTap) {
      if (!scope.active || scope.notificationPermission !== "granted") return false;
      if (!canRouteTap) return false;
      var snapshot = method("deepLinks.snapshot");
      if (!snapshot) return false;
      return Promise.resolve().then(function () {
        return snapshot.call(kit.deepLinks);
      }).then(function (value) {
        tapBaselineID = value ? value.id : null;
        return true;
      ...
      if (canRouteTap) {
        payload.path = NOTIFICATION_TAP_PATH;
        tapRouteAttached = true;
      }
      return show.call(kit.notifications, payload);
    }).then(function (shown) {
      if (!scope.active || shown === false) return false;
      if (shown !== true) throw new Error("Invalid notification result");
      scope.record("notifications", "pass", "granted", "shown");
      if (tapRouteAttached) {
        notificationTapArms.set(scope, { baselineID: tapBaselineID });
        scope.record("notification-tap", "running", "granted", "awaiting-matching-route");
        scope.statusText = "Đã gửi notification có route. PASS chỉ có nghĩa inbox nhận một route mới khớp; không xác nhận nguồn hay thao tác chạm vật lý.";
      } else {
