...
func TestApp1110AddsOnlySealedStudioSQLite(t *testing.T) {
	...
	legacy, err := composer.ComposeHTML([]byte(`<html data-kit-component="app@1.10.0" data-kit-as="$app"></html>`))
	if err != nil {
		t.Fatal(err)
	}
	current, err := composer.ComposeHTML([]byte(`<html data-kit-component="app@1.11.0" data-kit-as="$app"></html>`))
	if err != nil {
		t.Fatal(err)
	}
