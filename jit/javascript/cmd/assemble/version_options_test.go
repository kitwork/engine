...
func TestParseOptionsSelectsCanonicalClosedGraph(t *testing.T) {
	...
	if got.profile != javascript.ProfileHydrate || got.output != "" || got.canonicalDir != "dist" {
	...
	if got.services[1].name != "share" || got.services[1].version != "1.0.0" || got.services[1].path != "share.js" ||
		len(got.services[1].requires) != 1 || got.services[1].requires[0] != (javascript.ServiceVersion{Name: "storage", Version: "1.0.0"}) {
		t.Fatalf("share service = %#v", got.services[1])
	}
	if len(got.components) != 1 || got.components[0] != (javascript.ComponentVersion{Name: "dialog", Version: "1.0.0"}) {
