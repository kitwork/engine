...
func TestApp1120AddsOnlySealedStudioDatabase(t *testing.T) {
	...
	legacy, err := composer.ComposeHTML([]byte(`<html data-kit-component="app@1.11.0" data-kit-alias="$app"></html>`))
	if err != nil {
		t.Fatal(err)
	}
	current, err := composer.ComposeHTML([]byte(`<html data-kit-component="app@1.12.0" data-kit-alias="$app"></html>`))
	if err != nil {
		t.Fatal(err)
	}
