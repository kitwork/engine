...
; (function (document) {
  ...
  function prepareComponentTree(root) {
    ...
    root.querySelectorAll("template").forEach(function (template) {
      if (template.content) prepareComponentTree(template.content);
    });
  }
  ...
  function createInstance(descriptors, host) {
    var own = {};
    Object.keys(descriptors).forEach(function (name) {
  ...
  function ensureComponent(element) {
    ...
    try {
      current = createInstance(descriptors, element);
  ...
  function initialize(current) {
    ...
    try { core.observe(current.init.call(current.scope), current); }
...
;(function (document) {
  ...
  "component version as text show bind class model if for key".split(" ").forEach(function (name) {
    RESERVED[name] = true;
  });
...
;(function (document) {
  ...
  function morphChildren(current, incoming) {
    ...
    Array.prototype.slice.call(incoming.childNodes).forEach(function (incomingChild) {
      ...
      var match = key ? keyed.get(key) : positionalMatch(cursor);
      if (!match || used.has(match) || !compatible(match, incomingChild)) match = null;
      if (!match) {
  ...
  function morphElement(current, incoming) {
    ...
    if (current.localName === "template" && current.content && incoming.content) {
      morphChildren(current.content, incoming.content);
    } else morphChildren(current, incoming);
  ...
  function morphNode(current, incoming) {
    ...
    return morphElement(current, incoming);
  }
  ...
  function morph(currentRoot, incomingRoot) {
    ...
    var result = morphNode(currentRoot, incoming);
...
; (function (global, document) {
  ...
  function commit(incoming, url, options) {
    ...
    if (!sameProfile(incoming, url.href) || !knownComponents(incoming) ||
  ...
  function visit(source, options) {
    ...
    return global.fetch(url.href, {
      ...
      if (sequence !== visitSequence || controller.signal.aborted || error && error.name === "AbortError") {
        return false;
      }
      core.report(error);
      hardNavigate(url.href);
...
; (function (global, document) {
  ...
  var graph = { id: "71d305fe38382ab01cc99ed9bf5a20a0cb98bb16a436be889413b63c423a5eef", profile: "hydrate", components: components };
  if (core.reuse) {
  ...
  try {
    ...
    var kit = Object.freeze({ version: core.version, component: core.component });
    ; (function (kit) {
;(function () {
"use strict";
...
; (function (global, document) {
  ...
  if (typeof core.component !== "function" || typeof core.render !== "function" ||
    typeof core.installEvents !== "function") {
    delete document[ASSEMBLY];
  ...
  var properties = {
    ...
    component: { value: core.component, enumerable: true },
    [core.install]: { value: core.version }
  };
  if (core.graph) properties[GRAPH] = { value: core.graph };
  Object.defineProperties(kit, properties);
