...
; (function (document) {
  ...
  function component(name, definition) {
    ...
    core.registry.set(name, componentDescriptors(name, definition, core.graph));
...
; (function (global, document) {
  ...
  function stagedCandidate(incoming, responseURL) {
    ...
    for (var index = 0; index < scripts.length; index++) {
      ...
      if (!match || match[1] !== hash || !url || new URL(url).origin !== global.location.origin ||
  ...
  function visit(source, options) {
    ...
    // A popstate has already activated the destination history entry. Writing
    // the still-rendered page's viewport now would corrupt that destination.
    ...
    return Promise.resolve(request).then(function (response) {
      ...
      try {
        ...
        if (transition) {
          ...
          // Component packages are registration-only, but a malformed package
          // can still synchronously start a newer visit while materializing.
          // Never let the superseded outer visit commit its stale document.
          if (!currentVisit(visitRecord)) {
            rollback();
            return false;
          }
...
; (function (global, document) {
  ...
  var graph = { id: "fe72d0fb5a9ab0b72bd963f9983a8e828a157449d835c470d236c9969dff86f0", profile: "hydrate", services: services, components: components, actions: actions, grants: grants };
  if (core.reuse) {
