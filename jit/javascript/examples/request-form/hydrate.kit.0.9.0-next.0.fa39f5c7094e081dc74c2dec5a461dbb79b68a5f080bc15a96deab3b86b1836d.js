...
;(function (document) {
  ...
  function validateScopeTree(root) {
    ...
    root.querySelectorAll("template").forEach(function (template) {
      if (template.content) validateScopeTree(template.content);
    });
    return true;
  }
...
; (function (document) {
  ...
  function nearest(element) {
    while (element) {
      if (element.nodeType === 1 &&
        (element.hasAttribute("data-kit-component") || element.hasAttribute("data-kit-scope"))) return element;
  ...
  function componentElements(root) {
    ...
    root.querySelectorAll(BOUNDARIES).forEach(function (element) { output.push(element); });
    return output;
  }
...
;(function (document) {
  ...
  "component scope version as retain ignore text show bind class model if for key".split(" ").forEach(function (name) {
    RESERVED[name] = true;
  });
...
; (function (global, document) {
  ...
  var graph = { id: "5e0b8bbb64861bbb1cea410e60156f9d5a67f019d864b06e78caabc606702230", profile: "hydrate", services: services, components: components };
  if (core.reuse) {
