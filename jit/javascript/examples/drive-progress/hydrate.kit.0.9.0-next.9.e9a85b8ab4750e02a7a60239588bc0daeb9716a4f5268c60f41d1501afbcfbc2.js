...
; (function (document) {
  ...
  function installComponentGraph(source) {
    ...
    localRegistry.forEach(function (_, name) {
      if (OWN.call(graph.components, name)) {
        graphError("local component \"" + name + "\" conflicts with a managed component");
      }
    });
    core.graph = graph;
  ...
  function componentMetadata(element, shouldReport) {
    ...
    var graphIdentity = core.graph || null;
    var entry = metadata.get(element);
    if (entry && entry.componentSource === componentSource && entry.versionSource === versionSource &&
      entry.localSource === localSource && entry.graphIdentity === graphIdentity) {
    ...
    if (hasLocal) {
      ...
      if (hasVersion) {
        return metadataError(element, entry,
          "data-kit-local components cannot use data-kit-version", shouldReport);
      }
      if (element.hasAttribute("data-kit-retain")) {
        return metadataError(element, entry,
          "data-kit-local components cannot use data-kit-retain", shouldReport);
      }
      if (core.graph && OWN.call(core.graph.components, name)) {
        return metadataError(element, entry,
          "local component \"" + name + "\" conflicts with the installed graph", shouldReport);
      }
      entry.value = { name: name, version: null, local: true };
      metadata.set(element, entry);
      return entry.value;
    }
    if (core.graph && !OWN.call(core.graph.components, name)) {
    ...
    entry.value = { name: name, version: version, local: false };
    metadata.set(element, entry);
    return entry.value;
  }
  ...
  function assertComponentGraph() {
    ...
    localRegistry.forEach(function (_, name) {
      if (OWN.call(core.graph.components, name)) {
        throw new Error("KitJS: local component \"" + name + "\" conflicts with the installed graph");
      }
    });
  ...
  function component(name, definition) {
    ...
    if (!managed && componentHandoff && OWN.call(componentHandoff.graph.components, name)) {
      throw new Error("KitJS: local component \"" + name + "\" conflicts with the pending graph");
    }
    ...
    if (!managed && localRegistry.size >= core.cacheLimit) {
      throw new Error("KitJS: local component registry limit exceeded");
    }
  ...
  function beginComponentHandoff(graphSource, deliverySource) {
    ...
    Object.keys(targetGraph.components).forEach(function (name) {
      if (localRegistry.has(name)) {
        throw new Error("KitJS: component handoff conflicts with local component \"" + name + "\"");
      }
      if (OWN.call(core.graph.components, name) &&
  ...
  function reportMissingLocal(element, name) {
    ...
    core.report(new ReferenceError("KitJS: local component \"" + name +
  ...
  function auditLocalComponents() {
    ...
    document.querySelectorAll("[data-kit-component][data-kit-local]").forEach(function (element) {
      if (core.ignoredForRuntime(element)) return;
      var request = componentMetadata(element, false);
      if (request && request.local && !definitions.has(request.name)) {
  ...
  function ensureComponent(element) {
    ...
    var descriptors = request ?
      (request.local && core.graph ? localRegistry : core.registry).get(request.name) : Object.create(null);
    if (request && !descriptors) {
      if (request.local && localAuditComplete) reportMissingLocal(element, request.name);
      return null;
    }
    try {
  ...
  function resolveAppService(alias, current, serviceName) {
    if (alias !== "$app" || !current || !current.componentIdentity ||
      current.componentIdentity.local || current.componentIdentity.name !== "app" ||
  ...
  function resolveAppLoader(dependent, leaf) {
    ...
    if (!identity || identity.local || identity.name !== "app" || identity.alias !== "$app" ||
...
;(function (document) {
  ...
  function componentCompatible(current, incoming) {
    ...
    if (currentName === null && incomingName === null) {
...
; (function (global, document) {
  ...
  function knownComponents(incoming) {
    if (!incoming || !incoming.documentElement || !incoming.body ||
      !core.registry || typeof core.registry.has !== "function" ||
  ...
  function knownComponentsForGraph(incoming, graph) {
    if (!incoming || !incoming.documentElement || !incoming.body || !graph || !graph.components) return false;
    var components = [];
    var root = incoming.documentElement;
    if (root.hasAttribute("data-kit-component") || root.hasAttribute("data-kit-version")) {
      components.push(root);
    ...
    return components.every(function (element) {
      if (!element.hasAttribute("data-kit-component") ||
        String(element.localName || "").toLowerCase() === "template") return false;
      var name = String(element.getAttribute("data-kit-component") || "").trim();
      if (!Object.prototype.hasOwnProperty.call(graph.components, name)) return false;
      if (!element.hasAttribute("data-kit-version")) return true;
      return String(element.getAttribute("data-kit-version") || "").trim() === graph.components[name];
    });
  }
  ...
  function compatibleDocumentBoundary(incoming) {
    ...
    return ["data-kit-component", "data-kit-version", "data-kit-as", "data-kit-scope"].every(function (name) {
...
; (function (global, document) {
  ...
  var graph = { id: "efeff025ced573157117146d8b9dab3b94881ef691bd644c873647cbde4ae8ac", profile: "hydrate", services: services, components: components, actions: actions, grants: grants };
  if (core.reuse) {
