...
func TestBrowserHydrateDriveNavigation(t *testing.T) {
	...
	server := httptest.NewServer(http.HandlerFunc(func(response http.ResponseWriter, request *http.Request) {
		...
		case "/drive.html":
			writeHydrateHTML(response, hydrateDriveInitialDocument)
		case "/slow":
			time.Sleep(300 * time.Millisecond)
			writeHydrateHTML(response, hydrateDriveRouteDocument("Slow", "Slow", "/assets/slow.css"))
		case "/fast":
			writeHydrateHTML(response, hydrateDriveRouteDocument("Fast", "Fast", "/assets/fast.css"))
		case "/search":
...
func hydrateDriveRouteDocument(title, route, stylesheet string) string {
...
<head>
  ...
  <title>%s</title>
  <script>globalThis.__incomingScriptRuns = (globalThis.__incomingScriptRuns || 0) + 1;</script>
  <script src="/hydrate.kit.js?v=contract"></script>
</head>
<body>
  %s
  <script data-incoming-script>globalThis.__incomingScriptRuns = (globalThis.__incomingScriptRuns || 0) + 10;</script>
</body>
</html>`, title, title, strings.ToLower(title), stylesheet, title, hydrateDriveShell(route))
}
...
var hydrateDriveInitialDocument = fmt.Sprintf(`<!doctype html>
<html lang="en">
<head>
  <meta charset="utf-8">
  ...
  <style data-kit-head>#route-main { outline: none; }</style>
  <title>Initial</title>
  <script>globalThis.__initialScriptRuns = (globalThis.__initialScriptRuns || 0) + 1;</script>
  <script src="/hydrate.kit.js?v=contract"></script>
</head>
<body>
  %s
...
</html>`, hydrateDriveShell("Initial"), browserHarness, hydrateDriveAssertions)
