...
func TestBrowserSyncInitCleanupAcrossStructureAndMorph(t *testing.T) {
	...
	server := httptest.NewServer(http.HandlerFunc(func(response http.ResponseWriter, request *http.Request) {
		...
		case "/cleanup-contract.html":
			writeCleanupHTML(response, cleanupLifecycleInitialDocument(assetPath))
		case "/cleanup-next":
			writeCleanupHTML(response, cleanupLifecycleNextDocument(assetPath))
		case "/cleanup-again":
			writeCleanupHTML(response, cleanupLifecycleAgainDocument(assetPath))
		default:
...
func cleanupLifecycleStatePrelude() string {
	return `<script>
...
func cleanupLifecycleInitialDocument(assetPath string) string {
...
</html>`, cleanupLifecycleStatePrelude(), assetPath, cleanupLifecycleRetainedHost(),
		cleanupLifecycleDisposableHosts(), browserHarness, cleanupLifecycleAssertions)
}

func cleanupLifecycleNextDocument(assetPath string) string {
	return fmt.Sprintf(`<!doctype html>
<html lang="en"><head><meta charset="utf-8"><title>Cleanup next</title><script src=%q></script></head>
<body><main id="route-main">%s<a id="cleanup-again" href="/cleanup-again">again</a></main></body></html>`,
		assetPath, cleanupLifecycleRetainedHost())
}

func cleanupLifecycleAgainDocument(assetPath string) string {
	return fmt.Sprintf(`<!doctype html>
<html lang="en"><head><meta charset="utf-8"><title>Cleanup again</title><script src=%q></script></head>
<body><main id="route-main">%s%s<a id="cleanup-next-again" href="/cleanup-next">next again</a></main></body></html>`,
		assetPath, cleanupLifecycleRetainedHost(), cleanupLifecycleDisposableHosts())
}
...
<body>
  ...
  (function () {
    ...
    function collect(refs, controlRefs, pass) {
      ...
      if (alive(refs) !== 0) fail("runtime retained a directly removed cleanup owner");
      finish("passed");
    }
          ...
          try {
            ...
            setTimeout(function () {
              try {
                ...
                if (!host.isConnected) fail("same-document move disconnected its host");
                host.remove();
                host = null;
                waitFor(function () { return globalThis.__cleanupDirect.cleanup === 1; },
                  ...
                  try {
                    ...
                    setTimeout(function () { collect([hostRef, scopeRef], controls(), 0); }, 0);
