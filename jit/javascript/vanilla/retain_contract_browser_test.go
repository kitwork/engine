...
<body data-stage="initial">
  <div id="retain-parent-a">
    <section data-test-host="primary" data-kit-retain="primary" data-kit-component="retain-one" data-kit-version="1.0.0" data-kit-as="$one" data-server="old-primary">
    ...
    <section data-test-host="changed" data-kit-retain="change-old" data-kit-component="retain-one" data-kit-version="1.0.0" data-kit-as="$changed"><span>changed old</span></section>
  </div>
  <div id="retain-parent-b">
    <section data-test-host="secondary" data-kit-retain="secondary" data-kit-component="retain-two" data-kit-version="1.0.0" data-kit-as="$two"><span>secondary old</span></section>
	<section data-test-host="versioned" data-kit-retain="versioned" data-kit-component="retain-one" data-kit-as="$versioned"><span>version old</span></section>
    <section data-test-host="removed" data-kit-retain="removed" data-kit-component="retain-one" data-kit-version="1.0.0" data-kit-as="$removed"><span>remove old</span></section>
  </div>
`)
...
__runStandaloneKitTest(async function () {
  ...
  var moved = parsedBody(
    '<div id="retain-parent-a">' +
      '<section data-test-host="removed" data-kit-retain="removed" data-kit-component="retain-one" data-kit-version="1.0.0" data-kit-as="$removed"><span>remove moved</span></section>' +
      '<section data-test-host="versioned" data-kit-retain="versioned" data-kit-component="retain-one" data-kit-as="$versioned"><span>version moved</span></section>' +
    '</div>' +
    '<div id="retain-parent-b" data-layout="new">' +
      '<section data-test-host="secondary" data-kit-retain="secondary" data-kit-component="retain-two" data-kit-version="1.0.0" data-kit-as="$two" data-server="new-secondary"><span>secondary moved</span></section>' +
      '<section data-test-host="changed" data-kit-retain="change-old" data-kit-component="retain-one" data-kit-version="1.0.0" data-kit-as="$changed"><span>changed moved</span></section>' +
      '<section data-test-host="primary" data-kit-retain="primary" data-kit-component="retain-one" data-kit-version="1.0.0" data-kit-as="$one" data-server="new-primary">' +
  ...
  primary.setAttribute("data-kit-component", "retain-two");
  primary.removeAttribute("data-kit-version");
  primary.setAttribute("data-kit-as", "$mutatedOne");
  var matchingMutation = parsedBody(
    '<section data-test-host="primary-mutated" data-kit-retain="primary" data-kit-component="retain-two" data-kit-as="$mutatedOne">' +
  ...
  var incompatible = parsedBody(
    '<div id="retain-parent-a">' +
      '<section data-test-host="versioned-next" data-kit-retain="versioned" data-kit-component="retain-one" data-kit-version="1.0.0" data-kit-as="$versioned"><span>version next</span></section>' +
    '</div>' +
    '<div id="retain-parent-b">' +
      '<section data-test-host="changed-next" data-kit-retain="change-next" data-kit-component="retain-one" data-kit-version="1.0.0" data-kit-as="$changed"><span>changed next</span></section>' +
      '<section data-test-host="primary-next" data-kit-retain="primary" data-kit-component="retain-one" data-kit-version="1.0.0" data-kit-as="$one"><output data-kit-text="count">primary fresh</output></section>' +
      '<section data-test-host="secondary-next" data-kit-retain="secondary" data-kit-component="retain-two" data-kit-version="1.0.0" data-kit-as="$twoNext"><output data-kit-text="count">secondary fresh</output></section>' +
    '</div>');
  ...
  assert(host("versioned") !== versioned && !versioned.isConnected && scope(versioned) === undefined,
    "version mismatch retained stale DOM or scope");
  ...
  <output id="retain-two-next-count" data-kit-text="count">fresh two</output>
	...
	server := httptest.NewServer(http.HandlerFunc(func(response http.ResponseWriter, request *http.Request) {
		...
		case "/retain-drive.html":
			writeRetainHTML(response, retainDriveInitialDocument(assetPath))
		case "/retain-valid-current":
			if request.Header.Get("X-KitJS-Drive") == "1" {
				writeRetainHTML(response, retainDriveIncomingDocument(assetPath, "Valid current poison", validCurrentBody))
				return
			}
			writeRetainFallback(response, "current")
		case "/retain-identity":
			writeRetainHTML(response, retainDriveIncomingDocument(assetPath, "Identity committed", identityBody))
		default:
			...
			if request.Header.Get("X-KitJS-Drive") == "1" {
				writeRetainHTML(response, retainDriveIncomingDocument(assetPath, "Retain poison", body))
				return
			}
...
func retainDriveInitialDocument(assetPath string) string {
...
</body></html>`, assetPath, retainDriveHosts("Initial", true), browserHarness, retainDriveAssertions)
}
...
func retainDriveIncomingDocument(assetPath, title, body string) string {
	return fmt.Sprintf(`<!doctype html>
<html lang="poisoned"><head>
  ...
  %s
  <script>globalThis.__retainIncomingScript = (globalThis.__retainIncomingScript || 0) + 1;</script>
</body></html>`, html.EscapeString(title), assetPath, body)
}
...
`)}},
	...
	server := httptest.NewServer(http.HandlerFunc(func(response http.ResponseWriter, request *http.Request) {
		...
		case assetPath:
			...
			_, _ = response.Write(artifact.Bytes())
		case "/retain-gc.html":
			writeRetainHTML(response, fmt.Sprintf(retainGCInitialDocument, assetPath))
		case "/retain-gc-next":
			writeRetainHTML(response, fmt.Sprintf(retainGCNextDocument, assetPath))
		default:
			http.NotFound(response, request)
		}
	}))
