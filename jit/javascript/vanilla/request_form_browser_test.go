...
const requestFormArtifactName = "hydrate.kit.0.8.0.f46bd5bdc1ea6b6cbfbd043c4ff0357389329ce3396adf92982020694548e54b.js"
...
func TestBrowserRequestFormExample(t *testing.T) {
	...
	pages := map[string][]byte{
		"/examples/request-form/index.html":  requestFormPage(t, "index.html", artifact.Name()),
		"/examples/request-form/design.html": requestFormPage(t, "design.html", artifact.Name()),
	}
	initial := injectRequestFormFetchSpy(t,
		injectBrowserAssertions(t, pages["/examples/request-form/index.html"], requestFormAssertions))
	directDesign := injectBrowserAssertions(t, pages["/examples/request-form/design.html"], requestFormDirectDesignAssertions)
...
func requestFormPage(t *testing.T, name, artifactName string) []byte {
	t.Helper()
...
func injectRequestFormFetchSpy(t *testing.T, source []byte) []byte {
	t.Helper()
	needle := []byte("  <script defer src=\"")
	index := bytes.Index(source, needle)
	if index < 0 {
		t.Fatal("request form page has no external artifact seam")
	}
	spy := []byte(`<script>
...
const requestFormDirectDesignAssertions = `__runStandaloneKitTest(async function () {
  ...
  assert(document.querySelectorAll("script[src]").length === 1,
    "direct design route did not use one sealed runtime");
...
const requestFormAssertions = `__runStandaloneKitTest(async function () {
  ...
  assert(document.querySelectorAll("script[src]").length === 1, "request form did not use one sealed runtime");
  ...
  assert(document.querySelectorAll("script[src]").length === 1,
    "incoming request form route added or fetched another runtime");
  assert(Object.keys(globalThis.kit).join(",") === "version,component,progress,request",
