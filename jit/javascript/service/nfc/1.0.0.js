...
// KitJS staged service: nfc@1.0.0
// Requires capabilities@1.0.0.
var OWN = Object.prototype.hasOwnProperty;
var OPTIONS = Object.freeze({ signal: true, timeoutMS: true });
...
if (typeof supportsCapability !== "function") {
  throw new Error("KitJS: nfc requires its exact capabilities boundary");
}
...
function serviceError(code, operation) {
  var messages = {
    DENIED: "NFC scanner permission was denied",
    CANCELLED: "NFC scan was cancelled",
    UNAVAILABLE: "NFC scanning is unavailable",
    TIMEOUT: "NFC scan timed out",
    OVERLOADED: "NFC scanner is busy",
    TOO_LARGE: "NFC code value is too large",
    FAILED: "NFC scan failed"
  };
  var error = new Error(messages[code] || messages.FAILED);
  objectDefineProperties(error, {
    name: { value: "KitNFCError" },
...
function nativeCall(operation, params, validator) {
  if (!nativeHost) return promiseReject(serviceError("UNAVAILABLE", "scan"));
  try {
    return promiseResolve(nativeHost.call("nfc." + operation, params)).then(
      function (value) {
        try { return validator(value); }
        catch (error) {
          if (error === TOO_LARGE_VALUE) throw serviceError("TOO_LARGE", "scan");
          throw serviceError("FAILED", "scan");
        }
      },
      function (error) { throw serviceError(errorCode(error), "scan"); }
    );
  } catch (error) {
    return promiseReject(serviceError(errorCode(error), "scan"));
  }
}
...
function safeOperation(value) {
  if (typeof value !== "string" || !regExpTest(OPERATION, value)) {
    throw new TypeError("invalid NFC operation");
  }
  return value;
}
...
function safeValue(value) {
  var d = exactObject(value, ["records"]);
  if (!d || !arrayIsArray(d.records.value)) throw new TypeError("Invalid NFC records");
  var input = d.records.value;
  var descriptors = objectGetOwnPropertyDescriptors(input);
  if (objectGetPrototypeOf(input) !== arrayPrototype || input.length > 32 || objectGetOwnPropertySymbols(input).length || objectKeys(descriptors).length !== input.length + 1) throw new TypeError("Invalid NFC array");
  var output = [], total = 0;
  for (var i = 0; i < input.length; i++) {
    var entry = descriptors[String(i)];
    if (!entry || !entry.enumerable || !OWN.call(entry, "value")) throw new TypeError("Invalid NFC array entry");
    var record = exactObject(entry.value, ["type", "value"]);
    if (!record) throw new TypeError("Invalid NFC record");
    var type = record.type.value, text = record.value.value;
    if (type !== "text" && type !== "url" || typeof text !== "string" || !wellFormed(text) || text.indexOf("\x00") !== -1 || utf8Length(text) > 4096) throw new TypeError("Invalid NFC payload");
    total += utf8Length(text);
    if (total > 32768) throw new TypeError("NFC payload is too large");
    output.push(objectFreeze({type: type, value: text}));
  }
  return objectFreeze({records: objectFreeze(output)});
}

function beginResult(value) {
  var descriptors = exactObject(value, ["operation"]);
  if (!descriptors) throw new TypeError("invalid NFC begin result");
...
function pollResult(value) {
  ...
  if (descriptors) {
    ...
    throw new TypeError("invalid NFC status");
  }
...
function plainOptions(value) {
  ...
  try {
    ...
    throw new TypeError("NFC scan options must be a plain object");
  }
  if (!value || prototype !== Object.prototype && prototype !== null || symbols.length) {
    throw new TypeError("NFC scan options must be a plain object");
  }
  ...
  objectKeys(descriptors).forEach(function (name) {
    if (!OWN.call(OPTIONS, name)) throw new TypeError("Unknown NFC scan option: " + name);
    var descriptor = descriptors[name];
    if (!descriptor.enumerable || !OWN.call(descriptor, "value") || descriptor.get || descriptor.set) {
      throw new TypeError("NFC scan options must not contain accessors");
    }
...
function available() {
  if (arguments.length !== 0) throw new TypeError("nfc.available does not accept parameters");
  if (!nativeHost) return promiseResolve(false);
  try {
    return promiseResolve(supportsCapability.call(capabilities, "nfc.scan")).then(
      function (value) {
...
function scan(input) {
  if (arguments.length > 1) throw new TypeError("nfc.scan accepts at most one options object");
  var options = plainOptions(input);
  var timeout = options.timeoutMS === undefined ? 30000 : options.timeoutMS;
  if (!numberIsInteger(timeout) || timeout < 1000 || timeout > 60000) throw new TypeError("Invalid NFC timeout");
  var params = { timeoutMS: timeout };
  var signal = checkedSignal(options.signal);
  if (isAborted(signal)) return promiseReject(serviceError("CANCELLED", "scan"));
  if (!nativeHost || !setTimer || !clearTimer) {
    return promiseReject(serviceError("UNAVAILABLE", "scan"));
  }
  if (global.document && global.document.visibilityState === "hidden") return promiseReject(serviceError("CANCELLED", "scan"));
  if (activeScan) return promiseReject(serviceError("OVERLOADED", "scan"));
  ...
  function releaseOperation() {
    ...
    state.releasePending = new PromiseType(function (resolve) {
      ...
      try {
        timer = setTimer(function () { done(serviceError("TIMEOUT", "scan")); }, RELEASE_TIMEOUT_MS);
      } catch (_) { done(serviceError("UNAVAILABLE", "scan")); }
  ...
  function abort() {
    finish(false, serviceError("CANCELLED", "scan"), true);
  }

  function schedulePoll() {
    if (state.terminal) return;
    if (state.polls >= SCAN_MAX_POLLS) {
      finish(false, serviceError("TIMEOUT", "scan"), true);
      return;
    }
    try {
      ...
      finish(false, serviceError("UNAVAILABLE", "scan"), true);
    }
  }
  ...
  function poll() {
    ...
    nativeCall("poll", { operation: state.operation }, pollResult).then(function (value) {
      ...
      if (value.status === "cancelled") {
        finish(false, serviceError("CANCELLED", "scan"), false);
        return;
      }
      if (value.status === "failed") {
        finish(false, serviceError(value.code, "scan"), false);
        return;
      }
  ...
  try {
    state.beginTimer = setTimer(function () {
      ...
      if (!state.terminal) finish(false, serviceError("TIMEOUT", "scan"), true);
      else complete();
    }, BEGIN_TIMEOUT_MS);
  } catch (_) {
    state.beginPending = false;
  ...
  nativeCall("begin", params, function (value) {
    ...
    try {
      state.deadlineTimer = setTimer(function () {
        state.deadlineTimer = null;
        finish(false, serviceError("TIMEOUT", "scan"), true);
      }, Math.min(SCAN_TIMEOUT_MS, timeout + 5000));
    } catch (_) {
      finish(false, serviceError("UNAVAILABLE", "scan"), true);
      return;
    }
...
function status() {
  if (arguments.length) throw new TypeError("nfc.status accepts no arguments");
  return nativeCall("status", {}, function (value) {
    var d = exactObject(value, ["available", "enabled"]);
    if (!d || typeof d.available.value !== "boolean" || typeof d.enabled.value !== "boolean") throw new TypeError("Invalid NFC status");
    return objectFreeze({ available: d.available.value, enabled: d.enabled.value });
  });
}
