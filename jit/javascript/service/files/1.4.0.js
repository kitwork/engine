...
// KitJS staged service: files@1.4.0
...
var EXPORT_POLL_DELAY_MS = 250;
var EXPORT_TIMEOUT_MS = 10 * 60 * 1000;
var EXPORT_MAX_POLLS = 2400;
var FILE_HANDLE = /^[A-Za-z0-9_-]{32}$/;
var FILE_OPERATION = /^[A-Za-z0-9_-]{32}$/;
...
var uploadDescriptor = assembly && objectGetOwnPropertyDescriptor(assembly, UPLOAD_BOUNDARY);
var nativeUpload = uploadDescriptor && uploadDescriptor.value;
if (!assembly || !assembly.graph || assembly.graph.services.network !== "1.1.0" ||
  !uploadDescriptor || uploadDescriptor.get || uploadDescriptor.set || uploadDescriptor.configurable !== true ||
  typeof nativeUpload !== "function") throw new Error("KitJS: files requires its exact network upload boundary");
if (!delete assembly[UPLOAD_BOUNDARY] || OWN.call(assembly, UPLOAD_BOUNDARY)) throw new Error("KitJS: upload boundary did not close");
...
function safeOperation(value) {
  if (typeof value !== "string" || !regExpTest(FILE_OPERATION, value)) {
...
function pick(options) {
  ...
  var selected = new PromiseType(function (resolve, reject) {
    ...
    function finish(value, error, retainInput) {
      if (settled) return;
      settled = true;
      quiesce();
      // Android WebView may keep a selected File backed by its transient
      // content URI. Keep the input (and therefore that File) alive until the
      // FileVault import settles; removing it here can race Blob reads after
      // the system picker returns.
      if (!retainInput) removeInput();
  ...
  var imported = selected.then(function (file) {
  ...
  return imported.then(function (value) {
    detach();
    return value;
  }, function (error) {
    detach();
...
function exportDelay() {
  return new PromiseType(function (resolve) {
    setTimer(resolve, EXPORT_POLL_DELAY_MS);
  });
}

function bestEffortReleaseExport(operation) {
  nativeCall("releaseExport", { operation: operation }, function (value) {
    if (typeof value !== "boolean") throw new TypeError("invalid file export release result");
    return value;
  }).then(function () {}, report);
}

function exportFile(reference) {
  if (arguments.length !== 1) throw new TypeError("files.export expects one FileRef");
  var state = stateFor(reference);
  if (!setTimer || !clearTimer) return promiseReject(filesError("UNAVAILABLE", "export"));
  var control = { timedOut: false };
  var lifecycle = nativeCall("beginExport", { handle: state.handle }, function (value) {
    var descriptors = exactObject(value, ["operation"]);
    if (!descriptors) throw new TypeError("invalid file export operation");
    return safeOperation(descriptors.operation.value);
  }).then(function (operation) {
    var polls = 0;
    function poll() {
      if (control.timedOut) throw filesError("TIMEOUT", "export");
      polls++;
      return nativeCall("pollExport", { operation: operation }, function (value) {
        var descriptors = exactObject(value, ["state"]);
        if (!descriptors || descriptors.state.value !== "pending" && descriptors.state.value !== "writing" &&
          descriptors.state.value !== "completed" && descriptors.state.value !== "cancelled" &&
          descriptors.state.value !== "failed") {
          throw new TypeError("invalid file export state");
        }
        return descriptors.state.value;
      }).then(function (exportState) {
        if (exportState === "completed") {
          bestEffortReleaseExport(operation);
          return true;
        }
        ...
        if (exportState === "failed") {
          bestEffortReleaseExport(operation);
          throw filesError("FAILED", "export");
        }
        if (control.timedOut) throw filesError("TIMEOUT", "export");
        if (polls >= EXPORT_MAX_POLLS) throw filesError("TIMEOUT", "export");
        return exportDelay().then(poll);
      });
    }
    if (control.timedOut) throw filesError("TIMEOUT", "export");
    return poll();
  ...
  return new PromiseType(function (resolve, reject) {
    ...
    var timer = setTimer(function () {
      ...
      settled = true;
      reject(filesError("TIMEOUT", "export"));
    }, EXPORT_TIMEOUT_MS);
    lifecycle.then(function (value) {
      if (settled) return;
      settled = true;
      try { clearTimer(timer); }
      catch (_) { /* Terminal export settlement remains authoritative. */ }
      resolve(value);
    }, function (error) {
      if (settled) return;
      settled = true;
    ...
    } catch (_) { /* Private host details never escape files.export. */ }
    if (name === "KitFilesError" && operation === "export") throw error;
    throw filesError(errorCode(error), "export");
  });
}

function upload(reference, options) {
  if (arguments.length !== 2) throw new TypeError("files.upload expects a FileRef and options");
  var state = stateFor(reference);
  return nativeUpload(state.handle, options);
}
...
function installFileAdopter() {
  var graph = assembly && assembly.graph;
  if (!graph || !graph.services || graph.services.files !== "1.4.0" ||
    graph.services.camera !== "1.1.0") return;
  if (OWN.call(assembly, FILE_ADOPTER)) {
    throw new Error("KitJS: file adoption boundary is already installed");
  }
  Object.defineProperty(assembly, FILE_ADOPTER, {
    configurable: true,
    value: Object.freeze(function (value) {
      if (arguments.length !== 1) throw new TypeError("File adoption expects canonical metadata");
      return makeRef(metadata(value));
    })
  });
}
