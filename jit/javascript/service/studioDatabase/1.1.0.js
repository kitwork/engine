...
// KitJS staged service: studioDatabase@1.1.0
...
var STRUCTURE_FORMAT = "kitwork-studio-native-database-structure";
var TABLE_FORMAT = "kitwork-studio-native-database-table";
var QUERY_FORMAT = "kitwork-studio-native-database-query";
var QUERY_COUNT_FORMAT = "kitwork-studio-native-database-query-count";
...
function objectSnapshot(value, budget, schemas) {
  ...
  if (budget.bytes > MAX_RESULT_BYTES) return null;
  var rawColumns = arrayDescriptors(descriptors.columns.value, 0, MAX_COLUMNS);
  var rawIndexes = arrayDescriptors(descriptors.indexes.value, 0, MAX_INDEXES);
  var rawForeignKeys = arrayDescriptors(descriptors.foreignKeys.value, 0, MAX_FOREIGN_KEYS);
  if (!rawColumns || !rawIndexes || !rawForeignKeys ||
    rawColumns.length === 0 && (rawIndexes.length !== 0 || rawForeignKeys.length !== 0)) return null;
  var shallow = rawColumns.length === 0;
  var columns = shallow ? objectFreeze(new ArrayType(0)) : columnsSnapshot(descriptors.columns.value, budget);
  var indexes = shallow ? objectFreeze(new ArrayType(0)) : indexesSnapshot(descriptors.indexes.value, budget);
  var foreignKeys = shallow ? objectFreeze(new ArrayType(0)) : foreignKeysSnapshot(descriptors.foreignKeys.value, budget);
...
function plainStructureOptions(value) {
  var descriptors = exactObject(value, ["schema", "type", "name", "catalogVersion"]);
  if (!descriptors) throw new TypeError("Database structure options must contain exactly schema, type, name, and catalogVersion");
  var schema = safeName(descriptors.schema.value);
  var type = descriptors.type.value;
  var name = safeName(descriptors.name.value);
  var catalogVersion = descriptors.catalogVersion.value;
  if (!schema || (type !== "table" && type !== "view") || !name ||
    !safeInteger(catalogVersion, 1, 2147483647)) {
    throw new TypeError("Database structure options are invalid");
  }
  return objectFreeze({
    schema: schema.value,
    type: type,
...
function structureResult(value, expected, state) {
  var descriptors = resultEnvelope(value, [
    "catalogVersion", "name", "type", "columns", "indexes", "foreignKeys", "durationMs", "cached"
  ], STRUCTURE_FORMAT, state, expected.schema);
  if (!descriptors || descriptors.catalogVersion.value !== expected.catalogVersion ||
    descriptors.name.value !== expected.name || descriptors.type.value !== expected.type ||
    !safeInteger(descriptors.durationMs.value, 0, MAX_DURATION_MS) ||
    typeof descriptors.cached.value !== "boolean") {
    throw new TypeError("invalid database structure result");
  }
  var budget = { bytes: 0 };
  var columns = columnsSnapshot(descriptors.columns.value, budget);
  ...
  if (!columns || !indexes || !foreignKeys) throw new TypeError("invalid database structure data");
  return objectFreeze({
    catalogVersion: expected.catalogVersion,
    schema: expected.schema,
    name: expected.name,
    ...
    foreignKeys: foreignKeys,
    durationMs: descriptors.durationMs.value,
    cached: descriptors.cached.value
  });
}
...
function queryCountResult(value, state) {
  var descriptors = resultEnvelope(value, ["totalRows", "durationMs"], QUERY_COUNT_FORMAT, state);
  if (!descriptors || !safeInteger(descriptors.totalRows.value, 0, MAX_SAFE_INTEGER) ||
    !safeInteger(descriptors.durationMs.value, 0, MAX_DURATION_MS)) {
    throw new TypeError("invalid database query count result");
  }
  return objectFreeze({
    totalRows: descriptors.totalRows.value,
    durationMs: descriptors.durationMs.value
  });
}
...
function catalog(reference) {
  ...
  return nativeCall("catalogList", { handle: state.handle }, "catalog", function (value) {
    return catalogResult(value, state);
  });
}

function structure(reference, input) {
  if (arguments.length !== 2) throw new TypeError("studioDatabase.structure expects a ConnectionRef and options");
  var state = referenceState(reference, false);
  var options = plainStructureOptions(input);
  return nativeCall("structure", {
    handle: state.handle,
    schema: options.schema,
    type: options.type,
    name: options.name,
    catalogVersion: options.catalogVersion
  }, "structure", function (value) { return structureResult(value, options, state); });
}
...
function queryCount(reference, sql) {
  if (arguments.length !== 2) throw new TypeError("studioDatabase.queryCount expects a ConnectionRef and SQL text");
  var state = referenceState(reference, false);
  var checked = safeText(sql, MAX_SQL_BYTES, false, false);
  if (!checked || stringTrim(sql) === "") throw new TypeError("Database query count must be non-empty text up to 32 KiB");
  return nativeCall("queryCount", { handle: state.handle, query: sql }, "queryCount", function (value) {
    return queryCountResult(value, state);
  });
}
