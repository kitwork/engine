...
// KitJS staged service: geolocation@1.0.0
// Requires capabilities@1.0.0.
var OWN = Object.prototype.hasOwnProperty;
var OPTIONS = Object.freeze({ signal: true, highAccuracy: true, timeoutMS: true, maximumAgeMS: true });
...
if (typeof supportsCapability !== "function") {
  throw new Error("KitJS: geolocation requires its exact capabilities boundary");
}
...
function serviceError(code, operation) {
  var messages = {
    ...
    CANCELLED: "Location request was cancelled",
    UNAVAILABLE: "Location is unavailable",
    TIMEOUT: "Location request timed out",
    OVERLOADED: "Location request is busy",
    TOO_LARGE: "Location result is too large",
    FAILED: "Location request failed"
  };
  var error = new Error(messages[code] || messages.FAILED);
  objectDefineProperties(error, {
    name: { value: "KitGeolocationError" },
...
function nativeCall(operation, params, validator) {
  if (!nativeHost) return promiseReject(serviceError("UNAVAILABLE", "getCurrentPosition"));
  try {
    return promiseResolve(nativeHost.call("geolocation." + operation, params)).then(
      function (value) {
        try { return validator(value); }
        catch (error) {
          if (error === TOO_LARGE_VALUE) throw serviceError("TOO_LARGE", "getCurrentPosition");
          throw serviceError("FAILED", "getCurrentPosition");
        }
      },
      function (error) { throw serviceError(errorCode(error), "getCurrentPosition"); }
    );
  } catch (error) {
    return promiseReject(serviceError(errorCode(error), "getCurrentPosition"));
  }
}
...
function safeOperation(value) {
  if (typeof value !== "string" || !regExpTest(OPERATION, value)) {
    throw new TypeError("invalid Geolocation operation");
  }
  return value;
}
...
function safeValue(value) {
  var d = exactObject(value, ["latitude", "longitude", "accuracy", "timestamp"]);
  if (!d) throw new TypeError("Invalid location result");
  var limits = {latitude: [-90,90], longitude: [-180,180], accuracy: [0,100000000], timestamp: [1,9007199254740991]};
  var output = {};
  objectKeys(limits).forEach(function (key) {
    var n = d[key].value;
    if (typeof n !== "number" || !numberIsFinite(n) || n < limits[key][0] || n > limits[key][1]) throw new TypeError("Invalid location field");
    if (key === "timestamp" && !numberIsSafeInteger(n)) throw new TypeError("Invalid location timestamp");
    output[key] = n;
  });
...
function beginResult(value) {
  ...
  if (!descriptors) throw new TypeError("invalid Geolocation begin result");
...
function pollResult(value) {
  ...
  if (descriptors) {
    ...
    throw new TypeError("invalid Geolocation status");
  }
...
function plainOptions(value) {
  ...
  try {
    ...
    throw new TypeError("Geolocation scan options must be a plain object");
  }
  if (!value || prototype !== Object.prototype && prototype !== null || symbols.length) {
    throw new TypeError("Geolocation scan options must be a plain object");
  }
  ...
  objectKeys(descriptors).forEach(function (name) {
    if (!OWN.call(OPTIONS, name)) throw new TypeError("Unknown Geolocation scan option: " + name);
    var descriptor = descriptors[name];
    if (!descriptor.enumerable || !OWN.call(descriptor, "value") || descriptor.get || descriptor.set) {
      throw new TypeError("Geolocation scan options must not contain accessors");
    }
...
function available() {
  if (arguments.length !== 0) throw new TypeError("geolocation.available does not accept parameters");
  if (!nativeHost) return promiseResolve(false);
  try {
    return promiseResolve(supportsCapability.call(capabilities, "geolocation.read")).then(
      function (value) {
...
function scan(input) {
  if (arguments.length > 1) throw new TypeError("geolocation.getCurrentPosition accepts at most one options object");
  var options = plainOptions(input);
  var high = options.highAccuracy === undefined ? false : options.highAccuracy;
  var timeout = options.timeoutMS === undefined ? 15000 : options.timeoutMS;
  var age = options.maximumAgeMS === undefined ? 0 : options.maximumAgeMS;
  if (typeof high !== "boolean" || !numberIsInteger(timeout) || timeout < 1000 || timeout > 60000 || !numberIsInteger(age) || age < 0 || age > 300000) throw new TypeError("Invalid location options");
  var params = { highAccuracy: high, timeoutMS: timeout, maximumAgeMS: age };
  var signal = checkedSignal(options.signal);
  if (isAborted(signal)) return promiseReject(serviceError("CANCELLED", "getCurrentPosition"));
  if (!nativeHost || !setTimer || !clearTimer) {
    return promiseReject(serviceError("UNAVAILABLE", "getCurrentPosition"));
  }
  if (global.document && global.document.visibilityState === "hidden") return promiseReject(serviceError("CANCELLED", "getCurrentPosition"));
  if (activeScan) return promiseReject(serviceError("OVERLOADED", "getCurrentPosition"));
  ...
  function releaseOperation() {
    ...
    state.releasePending = new PromiseType(function (resolve) {
      ...
      try {
        timer = setTimer(function () { done(serviceError("TIMEOUT", "getCurrentPosition")); }, RELEASE_TIMEOUT_MS);
      } catch (_) { done(serviceError("UNAVAILABLE", "getCurrentPosition")); }
  ...
  function abort() {
    finish(false, serviceError("CANCELLED", "getCurrentPosition"), true);
  }

  function schedulePoll() {
    if (state.terminal) return;
    if (state.polls >= SCAN_MAX_POLLS) {
      finish(false, serviceError("TIMEOUT", "getCurrentPosition"), true);
      return;
    }
    try {
      ...
      finish(false, serviceError("UNAVAILABLE", "getCurrentPosition"), true);
    }
  }
  ...
  function poll() {
    ...
    nativeCall("poll", { operation: state.operation }, pollResult).then(function (value) {
      ...
      if (value.status === "cancelled") {
        finish(false, serviceError("CANCELLED", "getCurrentPosition"), false);
        return;
      }
      if (value.status === "failed") {
        finish(false, serviceError(value.code, "getCurrentPosition"), false);
        return;
      }
  ...
  try {
    state.beginTimer = setTimer(function () {
      ...
      if (!state.terminal) finish(false, serviceError("TIMEOUT", "getCurrentPosition"), true);
      else complete();
    }, BEGIN_TIMEOUT_MS);
  } catch (_) {
    state.beginPending = false;
...
function permission() {
  if (arguments.length) throw new TypeError("geolocation.permission accepts no arguments");
  return nativeCall("permission", {}, function (value) {
    if (value !== "prompt" && value !== "granted" && value !== "denied") throw new TypeError("Invalid location permission");
    return value;
  });
}

kit.service("geolocation", {
  available: available,
  permission: permission,
  getCurrentPosition: scan
});
})(globalThis, kit);
