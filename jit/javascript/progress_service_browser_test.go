...
func TestBrowserProgressComponentSubscriptionLifecycle(t *testing.T) {
	...
	contractSource := []byte(progressLifecycleBrowserContractSource)
	scriptTags := progressLifecycleScriptTags(
		assetPath,
	...
	server := httptest.NewServer(http.HandlerFunc(func(response http.ResponseWriter, request *http.Request) {
		...
		case "/assets/progress-lifecycle-contract.js":
			response.Header().Set("Content-Type", "text/javascript; charset=utf-8")
			_, _ = response.Write(contractSource)
		case "/one.html":
			response.Header().Set("Content-Type", "text/html; charset=utf-8")
			_, _ = fmt.Fprintf(response, progressLifecycleFirstDocument, scriptTags)
		case "/two.html":
			response.Header().Set("Content-Type", "text/html; charset=utf-8")
			_, _ = fmt.Fprintf(response, progressLifecycleSecondDocument, scriptTags)
		default:
			http.NotFound(response, request)
		}
	}))
...
func progressLifecycleScriptTags(assetPath, artifactIntegrity, contractIntegrity string) string {
	return fmt.Sprintf(`<script defer src="%s" integrity="%s" crossorigin="anonymous"></script>
<script defer src="/assets/progress-lifecycle-contract.js" integrity="%s" crossorigin="anonymous"></script>`,
		assetPath, artifactIntegrity, contractIntegrity)
}
...
const progressLifecycleBrowserContractSource = browserHarness + `
__runStandaloneKitTest(async function () {
  var assert = __kitTestAssert;
