...
type Service struct {
	...
	// Actions is the closed set of methods authored HTML may invoke through
	// an aliased component that directly requires this service.
	Actions []string
	Source  []byte
}
...
var (
	...
	serviceActionPattern = regexp.MustCompile(`^[A-Za-z_$][A-Za-z0-9_$]*$`)
...
func Build(options BuildOptions) (Artifact, error) {
	...
	graphSource, err := componentGraphSource(options.Profile, graphID, services, components, componentRequires, scripts)
	if err != nil {
		return Artifact{}, err
	}
...
// validateDocumentOwners prevents two packages from owning the same
// document-level state. theme@2 predates the appearance service and directly
// owns the root dark class and persisted preference; theme@3 is its adapter.
func validateDocumentOwners(components []ComponentVersion, services []Service) error {
	hasLegacyTheme := false
	hasAppearance := false
	for _, component := range components {
		if component.Name == "theme" && component.Version == "2.0.0" {
			hasLegacyTheme = true
		}
	}
	for _, service := range services {
		if service.Name == "appearance" && service.Version == "1.0.0" {
			hasAppearance = true
		}
	}
	if hasLegacyTheme && hasAppearance {
		return fmt.Errorf("kitjs: component theme@2.0.0 conflicts with document owner appearance@1.0.0; use theme@3.0.0")
	}
	return nil
}
...
func componentGraphID(profile Profile, runtime, serviceRuntime []byte, services []Service, components []ComponentVersion, componentRequires []ComponentServiceRequirement, scripts []Script) string {
	hash := sha256.New()
	writeHashFrame(hash, []byte("kitjs-package-graph-v3"))
...
func componentGraphSource(profile Profile, graphID string, services []Service, components []ComponentVersion, componentRequires []ComponentServiceRequirement, scripts []Script) ([]byte, error) {
	...
	_, _ = output.WriteString(", services: services, components: components, actions: actions, grants: grants };\n")
	...
	_, _ = output.WriteString("    }\n")
	_, _ = output.WriteString("    core.graphValidated = true;\n")
