...
| `data-kit-element="..."` | `$element.name` | Đặt tên một DOM element cho `$element.name` trong ACTION của boundary sở hữu nó (host component / `data-kit-scope` gần nhất, không thì trang); boundary lồng không thấy nhau; tên thiếu → nullish. Element chỉ mở tập đọc/động từ đóng (`value`, `focus()`, `scrollIntoView()`, …), không ghi qua ref. |
