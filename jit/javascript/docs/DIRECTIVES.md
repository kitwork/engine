...
| `data-kit-seed="title"` / `data-kit-seed:value="user.email"` | The mirror of bind: DOM → state, **once** (at boot, and again only for an element that arrives later). Bare, it reads the element's text (trimmed) — or the JSON of a `<script type="application/json">`; with `:<name>` it reads that property/attribute by the bind groups in reverse (`open`/`checked` → boolean, `value` → string, or a number on a numeric input; anything else → the attribute string, absent → `null`). The right side is a **state target**, not an expression: a key, a dotted path (`user.email` creates its objects), or `list[]` — one entry per element in document order, the list rebuilt whenever a new member appears. The DOM's value wins over a scope literal; on a component host the field must be declared. The server reads the same seeds for its first paint. |
