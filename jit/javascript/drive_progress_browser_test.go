...
const driveProgressArtifactName = "hydrate.kit.1.0.0-rc.2.7fa9414fbcceecae6c5fcb24903d152e11e7de751f853188f9045826125e9806.js"
...
func TestDriveProgressExampleContract(t *testing.T) {
	...
	for _, route := range routes {
		...
		stableTag := `<script defer src="./` + driveProgressArtifactName + `" data-kit-drive="stable"></script>`
...
func TestBrowserDriveProgressExample(t *testing.T) {
	...
	contractSource := []byte(driveProgressSpySource + "\n" + browserHarness + "\n" + driveProgressAssertions)
	contractIntegrity := driveScriptIntegrity(contractSource)
	pages := map[string][]byte{
		"/examples/drive-progress/index.html": injectDriveProgressContract(t, rawPages["/examples/drive-progress/index.html"], contractIntegrity),
		"/examples/drive-progress/next.html":  injectDriveProgressContract(t, rawPages["/examples/drive-progress/next.html"], contractIntegrity),
	}
	initial := pages["/examples/drive-progress/index.html"]
	directPages := map[string][]byte{
		"/examples/drive-progress/index.html": injectBrowserAssertions(t, rawPages["/examples/drive-progress/index.html"], driveProgressDirectAssertions),
		"/examples/drive-progress/next.html":  injectBrowserAssertions(t, rawPages["/examples/drive-progress/next.html"], driveProgressDirectAssertions),
	}
...
func injectDriveProgressContract(t *testing.T, source []byte, integrity string) []byte {
	t.Helper()
	...
	tag := []byte(`<script defer src="/drive-progress-contract.js" integrity="` + integrity + `" crossorigin="anonymous"></script>
`)
	return append(append(append([]byte(nil), source[:index]...), tag...), source[index:]...)
}

const driveProgressSpySource = `
...
func TestDriveProgressArtifactNameIsVersioned(t *testing.T) {
	prefix := "hydrate.kit." + ReleaseVersion + "."
	digest := strings.TrimSuffix(strings.TrimPrefix(driveProgressArtifactName, prefix), ".js")
	if !strings.HasPrefix(driveProgressArtifactName, prefix) ||
