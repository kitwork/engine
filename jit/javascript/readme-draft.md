...
> **Historical design draft — non-normative.** This file is retained only for
> design history and is superseded by [`README.md`](README.md), current source,
> and executable tests. In particular, `kit.use()` and authored
> `mount()`/`unmount()` are not part of the current KitJS architecture.
