...
func TestBrowserDriveNavigationEventLifecycle(t *testing.T) {
	...
	contractSource := []byte(navigationEventRecorder + "\n" + browserHarness + "\n" + navigationEventLifecycleAssertions)
	...
	server := httptest.NewServer(http.HandlerFunc(func(response http.ResponseWriter, request *http.Request) {
		...
		case "/navigation-event-contract.js":
			response.Header().Set("Content-Type", "text/javascript; charset=utf-8")
			_, _ = response.Write(contractSource)
		case "/navigation-events.html":
			writeNavigationHTML(response, navigationEventInitialDocument(
				assetPath, assetIntegrity, contractIntegrity))
		case "/event-loaded":
			writeNavigationHTML(response, navigationRouteDocument(
				assetPath, assetIntegrity, contractIntegrity, "Loaded", "loaded"))
		case "/event-fast":
			writeNavigationHTML(response, navigationRouteDocument(
				assetPath, assetIntegrity, contractIntegrity, "Fast", "fast"))
		case "/event-fallback":
...
func navigationRouteDocument(assetPath, assetIntegrity, contractIntegrity, title, route string) string {
	return fmt.Sprintf(`<!doctype html>
<html lang="en"><head><meta charset="utf-8"><title>%s</title>
<script defer src=%q integrity=%q crossorigin="anonymous"></script>
<script defer src="/navigation-event-contract.js" integrity=%q crossorigin="anonymous"></script></head>
<body>%s</body></html>`, title, assetPath, assetIntegrity, contractIntegrity, navigationShell(route))
}

func navigationEventInitialDocument(assetPath, assetIntegrity, contractIntegrity string) string {
	return navigationRouteDocument(assetPath, assetIntegrity, contractIntegrity,
		"Navigation events", "initial")
}
...
func TestBrowserDriveNavigationStreamProgress(t *testing.T) {
	...
	assetPath := "/assets/" + artifact.Name()
	assetIntegrity := driveScriptIntegrity(artifact.Bytes())
	placeholderIntegrity := driveScriptIntegrity(nil)
	provisionalKnown := navigationStreamDocument(assetPath, assetIntegrity, placeholderIntegrity,
		"Known stream", "known", 64<<10)
	knownTotal := len([]byte(provisionalKnown))
	assertions := fmt.Sprintf(navigationStreamAssertionsFormat, knownTotal)
	contractSource := []byte(navigationEventRecorder + "\n" + browserHarness + "\n" + assertions)
	contractIntegrity := driveScriptIntegrity(contractSource)
	known := navigationStreamDocument(assetPath, assetIntegrity, contractIntegrity, "Known stream", "known", 64<<10)
	unknown := navigationStreamDocument(assetPath, assetIntegrity, contractIntegrity, "Unknown stream", "unknown", 32<<10)
	encoded := navigationStreamDocument(assetPath, assetIntegrity, contractIntegrity, "Encoded stream", "encoded", 32<<10)
	if len([]byte(known)) != knownTotal {
		t.Fatal("navigation stream fixture changed length after sealing its shared contract")
	}
	...
	server := httptest.NewServer(http.HandlerFunc(func(response http.ResponseWriter, request *http.Request) {
		...
		case assetPath:
			response.Header().Set("Content-Type", "text/javascript; charset=utf-8")
			_, _ = response.Write(artifact.Bytes())
		case "/navigation-progress-contract.js":
			response.Header().Set("Content-Type", "text/javascript; charset=utf-8")
			_, _ = response.Write(contractSource)
		case "/navigation-progress.html":
			writeNavigationHTML(response, navigationProgressInitialDocument(
				assetPath, assetIntegrity, contractIntegrity))
		case "/stream-known":
		...
		case "/stream-fast":
			writeNavigationHTML(response, navigationStreamDocument(
				assetPath, assetIntegrity, contractIntegrity, "Stream fast", "fast", 0))
		default:
			http.NotFound(response, request)
		}
	}))
...
func navigationStreamDocument(assetPath, assetIntegrity, contractIntegrity, title, route string, padding int) string {
	return fmt.Sprintf(`<!doctype html>
<html lang="en"><head><meta charset="utf-8"><title>%s</title>
<script defer src=%q integrity=%q crossorigin="anonymous"></script>
<script defer src="/navigation-progress-contract.js" integrity=%q crossorigin="anonymous"></script></head>
<body>%s<div hidden data-stream-padding>%s</div></body></html>`, title, assetPath, assetIntegrity,
		contractIntegrity, navigationProgressShell(route), strings.Repeat("x", padding))
}

func navigationProgressInitialDocument(assetPath, assetIntegrity, contractIntegrity string) string {
	return navigationStreamDocument(assetPath, assetIntegrity, contractIntegrity,
		"Navigation progress", "initial", 0)
}
