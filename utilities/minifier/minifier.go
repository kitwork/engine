...
// (HTML, CSS, JS, JSStrict, JSON, SVG, XML, Type) so callers are build-tag-agnostic:
...
// plain functions rather than a constructed object — the package name is the namespace. The
// compatibility helpers return the input unchanged on a parse error, so they can never emit
// corrupt output. JSStrict is the fail-closed exception: the default parser reports its error,
// while the stdminify build deliberately returns the input unchanged with no error because that
// build explicitly opts out of JavaScript minification.
package minifier
